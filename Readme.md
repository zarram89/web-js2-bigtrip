...
# Задача 7: Сложные компоненты (module6-task1)

Форма редактирования довольно сложный интерактивный компонент. Но это поведение — не часть бизнес-логики приложения. Это «бизнес-логика» самого компонента. Поэтому для реализации этой логики воспользуемся классом `AbstractStatefulView`.

## Описание задачи
1.  **AbstractStatefulView**: Унаследовать `EditPointView` от `AbstractStatefulView`.
2.  **Внутреннее состояние**: Реализовать логику обновления состояния компонента при взаимодействии пользователя с формой (смена типа, смена пункта назначения).
3.  **Перерисовка**: Реализовать перерисовку формы при изменении состояния.
4.  **Восстановление обработчиков**: Реализовать метод `_restoreHandlers` для восстановления обработчиков событий после перерисовки.

# Решение Задачи 7 (module6-task1)

Для выполнения задачи я выполнил следующие действия:

### 1. Рефакторинг EditPointView
*   Изменил наследование с `AbstractView` на `AbstractStatefulView`.
*   В конструкторе инициализирую состояние `_state` на основе переданной точки, используя статический метод `parsePointToState`.
*   Реализовал геттер `template`, который использует `_state` для генерации разметки.

### 2. Управление состоянием
*   Реализовал методы парсинга:
    *   `parsePointToState(point)`: копирует данные точки в состояние.
    *   `parseStateToPoint(state)`: копирует данные состояния обратно в объект точки.
*   Реализовал обработчики внутренних событий:
    *   `#typeChangeHandler`: обновляет тип точки в состоянии и сбрасывает выбранные опции. Вызывает `updateElement` для перерисовки.
    *   `#destinationChangeHandler`: находит выбранный пункт назначения по имени и обновляет `destination` id в состоянии. Вызывает `updateElement`.

### 3. Восстановление обработчиков
*   Реализовал метод `_restoreHandlers`, который вызывается автоматически после `updateElement`.
*   В этом методе заново навешиваются внутренние обработчики (смена типа, назначения) и внешние (submit, rollup), так как DOM-элемент был полностью заменен.

### 4. Проверка
*   Проект собирается и работает.
*   При смене типа точки (например, с Flight на Taxi) иконка и список доступных опций обновляются мгновенно.
*   При смене пункта назначения обновляется описание и фотографии.
*   Кнопка Save сохраняет текущее состояние формы.

---

# Задача 8: Flatpickr и форматирование дат (module6-task2)

В этом задании мы интегрируем библиотеку `flatpickr` для выбора даты и времени в форме редактирования, а также настроим корректное отображение длительности событий.

## Описание задачи
1.  **Flatpickr**: Установить и подключить библиотеку `flatpickr` (включая стили).
2.  **Интеграция**: Использовать `flatpickr` для полей ввода даты начала и окончания события в `EditPointView`.
3.  **Форматирование**: Реализовать корректный подсчет и отображение длительности события в `PointView` (формат: `DDD HH MM`, `HH MM`, `MM`).

# Решение Задачи 8 (module6-task2)

Для выполнения задачи я выполнил следующие действия:

### 1. Интеграция Flatpickr
*   Импортировал `flatpickr` и его стили в `src/view/edit-point-view.js`.
*   В `EditPointView` добавил приватные свойства `#datepickerFrom` и `#datepickerTo` для хранения экземпляров календаря.
*   Реализовал метод `#setDatepicker`, который инициализирует `flatpickr` на полях ввода даты.
    *   Настройки: `dateFormat: 'd/m/y H:i'`, `enableTime: true`.
    *   Для даты окончания установлено ограничение `minDate` (не раньше даты начала).
*   Добавил вызов `#setDatepicker` в конструктор и `_restoreHandlers`.
*   Переопределил метод `removeElement` для уничтожения экземпляров календаря при удалении компонента (во избежание утечек памяти).
*   Реализовал обработчики `onChange` для обновления состояния компонента при выборе даты.

### 2. Форматирование длительности
*   Обновил функцию `formatDuration` в `src/view/point-view.js`.
*   Теперь длительность отображается в формате:
    *   `01D 02H 30M` (если больше суток)
    *   `02H 30M` (если меньше суток, но больше часа)
    *   `30M` (если меньше часа)
*   Использовал `dayjs` для вычисления разницы в минутах и ручное форматирование для соответствия требованиям (с добавлением ведущих нулей).

### 3. Проверка
*   Проект собирается и работает.
*   В форме редактирования при клике на поля даты открывается календарь.
*   Выбор даты обновляет состояние.
*   В списке событий длительность отображается корректно (например, `01H 30M`).
