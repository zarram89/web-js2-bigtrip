...
# Задача 11: Создание, удаление и обратная связь (module8-task2)

В этом задании мы завершили интеграцию с сервером, реализовав создание и удаление точек, а также добавили обратную связь для пользователя.

## Описание задачи
1.  **Создание и удаление**:
    *   Реализовать методы `addPoint` (POST) и `deletePoint` (DELETE) в API.
    *   Обновлять интерфейс только после успешного ответа сервера.
2.  **Обратная связь**:
    *   Блокировать интерфейс во время запроса.
    *   Менять текст кнопок на "Saving..." и "Deleting...".
    *   В случае ошибки "покачивать" формой (эффект shake) и возвращать исходное состояние.

# Решение Задачи 11 (module8-task2)

### 1. API и Модель
*   **`PointsApiService`**: Добавлены методы `addPoint` и `deletePoint`.
*   **`PointsModel`**: Методы `addPoint` и `deletePoint` теперь асинхронные, вызывают API и ждут ответа.

### 2. View и Состояния
*   **`EditPointView`**:
    *   Добавлены состояния `isDisabled`, `isSaving`, `isDeleting`.
    *   Инпуты и кнопки блокируются при `isDisabled`.
    *   Текст кнопок меняется в зависимости от `isSaving` и `isDeleting`.
    *   Используется метод `shake` (из `AbstractView`) для визуализации ошибок.

### 3. Презентеры
*   **`BoardPresenter`**:
    *   `handleViewAction` стал асинхронным.
    *   Перед запросом вызывает `setSaving` или `setDeleting` у соответствующего презентера.
    *   При ошибке вызывает `setAborting`.
*   **`PointPresenter` / `NewPointPresenter`**:
    *   Реализованы методы `setSaving`, `setDeleting` (только для редактирования), `setAborting`.
    *   Реализованы методы `setSaving`, `setDeleting` (только для редактирования), `setAborting`.
    *   `setAborting` вызывает `shake` у компонента формы и сбрасывает состояние после анимации.

# Задача 12: Улучшение UI/UX (module8-task3)

В этом задании мы добавили информацию о путешествии в шапку сайта и улучшили работу фильтров.

## Описание задачи
1.  **Информация о путешествии**:
    *   Выводить маршрут (Город 1 — Город 2 — Город 3 или Город 1 — ... — Город N).
    *   Выводить даты путешествия (Начало — Конец).
    *   Выводить общую стоимость (Цена точек + Цена выбранных офферов).
    *   Пересчитывать информацию при любом изменении данных.
2.  **Фильтры**:
    *   Блокировать фильтры, если для них нет подходящих точек.

# Решение Задачи 12 (module8-task3)

### 1. Trip Info
*   **`TripInfoView`**: Компонент для отображения маршрута, дат и стоимости.
*   **`TripInfoPresenter`**:
    *   Подписан на изменения модели `PointsModel`.
    *   Сортирует точки по дате.
    *   Рассчитывает маршрут: если городов > 3, скрывает промежуточные за "...", иначе показывает все.
    *   Рассчитывает даты: от `dateFrom` первой точки до `dateTo` последней.
    *   Рассчитывает стоимость: сумма базовых цен точек + сумма цен всех выбранных офферов.

### 2. Фильтры
*   **`FilterPresenter`**: Рассчитывает количество точек для каждого фильтра.
*   **`FilterView`**: Если количество точек равно 0, добавляет атрибут `disabled` к радиокнопке фильтра.
